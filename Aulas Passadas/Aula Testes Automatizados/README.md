...
Material arquivado da última aula da UC. Foi o dia em que a conferência saiu do
olho e passou para o computador.

No dia anterior o jogo mudou de forma sem mudar de comportamento, e a conferência foi
toda no olho: jogar, chegar na fase certa, provocar a situação certa e olhar a
tela. Cinco vezes, uma por capítulo.

Nesta aula o computador passa a fazer essa conferência. Uma vez escrita, ela custa
...
Na refatoração a regra era que nenhum arquivo novo entrava. Aqui entra exatamente um,
...
[`codigo-base/`](codigo-base/). Eles são o jogo da refatoração com os cinco
...
Usar o código-base não entregava a atividade do dia. Ele apenas evita perder a
...
- **Composição:** a `BonusTemporario` é testada sem criar nenhum `Personagem`.
  A frase escrita na refatoração — "não sabe a que atributo pertence" — vira
...
A refatoração está em [`../Aula Refatoração/`](../Aula%20Refatora%C3%A7%C3%A3o/).

## Depois desta aula

A UC terminou. O fechamento é a revisão geral, em [`../../Hoje/`](../../Hoje/):
sessenta e seis questões sobre todo o conteúdo, e o capítulo 8 dela cobra
justamente o que foi dado aqui.

Todo o restante está em [`../`](../).
